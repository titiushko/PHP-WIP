...
*/
	function &_addHtmlPart(&$obj)
	{
		...
		if (is_object($obj)) {
			$res = $obj->addSubpart($this->html, $params);
            return $res;
		} else {
			$res = new Mail_mimePart($this->html, $params);
			return $res;
		}
	}
...
*/
	function &_addMixedPart()
	{
		...
		$new_mime = new Mail_mimePart('', $params);
		return $new_mime;
	}
...
*/
	function &_addAlternativePart(&$obj)
	{
		...
		if (is_object($obj)) {
			$res = $obj->addSubpart('', $params);
            return $res;
		} else {
			$res = new Mail_mimePart('', $params);
            return $res;
		}
	}
		...
		switch ($type) {
			...
			case 'smtp':
				...
				if (isset($this->return_path)) {
                    ...
                    // RS - 2006
                    if (isset($from[0]) and isset($from[0]->mailbox)) {
...
*/
	function getRFC822($recipients)
	{
		...
		$res = implode(CRLF, $headers) . CRLF . CRLF . $this->output;
        return $res;
	}
} // End of class.
?>
