...
***************************************/
				...
				if(isset($this->var_names[$file_id]) AND count($this->var_names[$file_id]) > 0){
					for($i=0; $i<count($this->var_names[$file_id]); $i++){
						...
						if(is_long(strpos($this->files[$file_id], $this->start.$temp_var.$this->end))){
							global $$temp_var;
							$this->files[$file_id] = str_replace($this->start.$temp_var.$this->end, $this->var_values[$temp_var], $this->files[$file_id]);
	...
	***************************************/
		function parse_loop($file_id, $array_name){
			...
			if($loop_code != ''){
				$new_code = '';
				for($i=0; $i<count($this->var_values[$array_name]); $i++){
					$temp_code = $loop_code;
					while(list($key,) = each($this->var_values[$array_name][$i])){

						$temp_code = str_replace($this->start.$key.$this->end,$this->var_values[$array_name][$i][$key], $temp_code);
					}
					$new_code .= $temp_code;
				}
               ...
               function parse_if($file_id, $array_name){
                   ...
                   for($i=0; $i<count($var_names); $i++){
                        ...
                        if($if_code != ''){
                                ...
                                if(isset($this->var_values[$var_names[$i]]))
                                    $new_code = $if_code;
